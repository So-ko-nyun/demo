...
class VMwareCrawler(VMwareCrawlerAbs):
    ...
    def doCrawlingToday(self):
        ...
        serverUrl = "http://localhost:8081/uerp/v1/support/vmware/docs/knowledgebase"
        detail_data = startCrawlsToday(baseUrl, serverUrl)
        # list_to_json(detail_data)
        # save_json_to_file(detail_data, path, filename) json파일로 저장하고 싶은 경우 주석해제
        print("success")

        return detail_data

    def doCrawling(self, startDate, endDate):
        path = "/Users/chujiu/Coding/DataCrawling/BeautifulSouppp/Github_practice/kb-sync/src/Json_data"
        filename = "ExampleData_test.json"
...
startDate = "2024-7-17"
endDate = "2024-07-19"
# Swagger 주소: http://localhost:8081/docs#/


start_time = time.time()
hi = VMwareCrawler(baseUrl, dataUri, authUri, username, password)
oTime = hi.doCrawling(startDate, endDate)
# doToday = hi.doCrawlingToday()
end_time = time.time()
